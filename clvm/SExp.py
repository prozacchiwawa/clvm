...
def to_sexp_type(
    ...
    if isinstance(v, tuple):
        return CLVMObject((
            to_sexp_type(v[0], count + 1),
            to_sexp_type(v[1], count + 1),
        ))

    if isinstance(v, list):
        result = CLVMObject(NULL)
        for r in map(lambda u: to_sexp_type(u, count + 1), reversed(v)):
            result = CLVMObject((r, result))
        return result
